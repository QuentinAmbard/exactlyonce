...
object ExactlyOnceTestTransform extends App {
  ...
  kafkaStream.transform{ rdd =>
    println(s"-------------------------")
    rdd.asInstanceOf[HasOffsetRanges].offsetRanges.foreach{range => println(s"SAVE OFFSET RANGES: p:${range.partition} => [${range.fromOffset}-${range.untilOffset}]")}
    rdd
  }.foreachRDD(rdd => {
